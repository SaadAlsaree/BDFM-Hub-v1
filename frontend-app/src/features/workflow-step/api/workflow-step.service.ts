...
export const workflowStepService = {
    ...
    async createWorkflowStep(payload: WorkflowStepInput) {
        try {
            ...
            const response = await axiosClient.post(`${baseUrl}/Workflow/CreateWorkflowStep`, payload);
    ...
    async logRecipientInternalAction(payload: LogRecipientInternalActionInput) {
        try {
            const response = await axiosClient.post(`${baseUrl}/Workflow/LogRecipientInternalAction`, payload);
            if (response.status >= 400) {
                // console.error('Error creating role:', response.statusText);
                return null;
            }
            return response.data as IResponse<LogRecipientInternalActionInput> || null;
        } catch (error) {
            // console.log(error);
            return null;
        }
    ...
    async updateStatus(payload: UpdateStatusInput) {
        try {
            const response = await axiosClient.patch(`${baseUrl}/Workflow/UpdateStatus/UpdateStatus`, payload);
            if (response.status >= 400) {
                // console.error('Error creating role:', response.statusText);
                return null;
            }
    ...
    async completeWorkflowStep(id: string) {
        try {
            const response = await axiosClient.put(`${baseUrl}/Workflow/CompleteWorkflowStep/id/${id}`);
            if (response.status >= 400) {
                // console.error('Error creating role:', response.statusText);
                return null;
            }
    ...
    async createBulkWorkflowSteps(payload: WorkflowStepBulkInsert) {
        try {
            const response = await axiosClient.post(`${baseUrl}/Workflow/CreateBulkWorkflowSteps`, payload);
            return response.data as IResponse<boolean> || null;
        } catch (error) {
            return null;
        }
