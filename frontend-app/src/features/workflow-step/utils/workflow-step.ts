import { z } from "zod";
import { ActionTypeEnum, InternalActionTypeEnum, RecipientTypeEnum, WorkflowStepStatus } from "../types/workflow-step";
...
export const LogRecipientInternalActionInputSchema = z.object({
    ...
    actionTimestamp: z.coerce.date().min(new Date(), "Action timestamp must be in the future"),
...
export const WorkflowStepBulkInsertSchema = z.object({
    correspondenceId: z.string().min(1, "Correspondence ID is required"),
    workflowSteps: z.array(WorkflowStepBulkItemSchema).min(1, "At least one workflow step is required")
});

export type WorkflowStepInputFormData = z.infer<typeof WorkflowStepInputSchema>;
export type LogRecipientInternalActionInputFormData = z.infer<typeof LogRecipientInternalActionInputSchema>;
export type WorkflowStepBulkItemFormData = z.infer<typeof WorkflowStepBulkItemSchema>;
export type WorkflowStepBulkInsertFormData = z.infer<typeof WorkflowStepBulkInsertSchema>;
export type UpdateWorkflowStepStatusFormData = z.infer<typeof UpdateWorkflowStepStatusSchema>;
