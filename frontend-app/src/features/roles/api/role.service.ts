...
import { IRoleDetails, IRoleList, IRolePayload } from '@/features/roles/types/role';
...
export const roleService = {
    async getRoles(query: Record<string, any>) {
        try {
            const response = await axiosInstance.get(`${baseUrl}/Role/GetRoleList`, { params: query });
    ...
    async getRoleById(id: string) {
        try {
            ...
            const response = await axiosInstance.get(`${baseUrl}/Role/GetRoleById/${id}`);
    ...
    async createRole(role: IRolePayload) {
        try {
            const response = await axiosClient.post(`${baseUrl}/Role/CreateRole`, role);
    ...
    async updateRole(role: IRolePayload) {
        try {
            ...
            const response = await axiosClient.put(`${baseUrl}/Role/UpdateRole`, role);
    ...
    async updateRoleStatus(id: string, status: number) {
        try {
            ...
            const response = await axiosClient.patch(`${baseUrl}/Role/ChangeStatus/ChangeStatus`, request);
