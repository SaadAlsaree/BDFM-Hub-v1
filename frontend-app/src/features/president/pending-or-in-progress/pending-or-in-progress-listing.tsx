...
import { columns } from '../my-pending-or-in-progress/inbox-list-tables/columns';
import PendingOrInProgressListTable from './inbox-list-tables';

export default async function PendingOrInProgressListing() {
  ...
  const response =
    await correspondenceService.getPendingOrInProgressCorrespondences(filters);

  const totalItems = response?.data?.totalCount || 0;
  const pendingOrInProgressItems = (response?.data?.items || []) as InboxList[];

  return (
    <PendingOrInProgressListTable<InboxList, unknown>
      data={pendingOrInProgressItems}
