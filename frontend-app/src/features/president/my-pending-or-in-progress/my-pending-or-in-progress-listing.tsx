...
import MyPendingOrInProgressListTable from './inbox-list-tables';

export default async function MyPendingOrInProgressListing() {
  ...
  const response =
    await correspondenceService.getMyPendingOrInProgressCorrespondences(
      filters
    );

  const totalItems = response?.data?.totalCount || 0;
  const myPendingOrInProgressItems = (response?.data?.items ||
    []) as InboxList[];

  return (
    <MyPendingOrInProgressListTable<InboxList, unknown>
