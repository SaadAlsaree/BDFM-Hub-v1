...
import NotCompletedListTable from './inbox-list-tables';

export default async function NotCompletedListing() {
  ...
  const response =
    await correspondenceService.getNotCompletedCorrespondences(filters);

  const totalItems = response?.data?.totalCount || 0;
  const notCompletedItems = (response?.data?.items || []) as InboxList[];

  return (
    <NotCompletedListTable<InboxList, unknown>
