import { z } from "zod";
import { CustomWorkflowDetails, CorrespondenceTypeEnum, ActionTypeEnum, CustomWorkflowTargetTypeEnum } from "../types/customWorkflow";
...
export const CustomWorkflowStepListSchema = z.object({
    ...
    defaultDueDateOffsetDays: z.number().int().min(0, "Due date offset must be non-negative"),
    createAt: ISODateSchema,
    lastUpdateAt: ISODateSchema,
...
export const CreateWorkflowPayloadSchema = z.object({
    ...
    (data) => {
        ...
        if (!data.id) {
            return data.workflowName && data.triggeringUnitId && data.triggeringCorrespondenceType !== undefined;
        }
        return true;
    },
    {
        message: "For new workflows, workflowName, triggeringUnitId, and triggeringCorrespondenceType are required",
    }
);

export const CreateWorkflowStepPayloadSchema = z.object({
    id: UUIDSchema.optional(),
    ...
    defaultDueDateOffsetDays: z.number().int().min(0, "Due date offset must be non-negative").optional(),
}).refine(
    (data) => {
        // For create operations, require essential fields
        if (!data.id) {
    ...
    {
        message: "For new workflow steps, workflowId, stepOrder, actionType, targetType, and targetIdentifier are required",
    }
);
...
export type ValidatedCustomWorkflowDetails = z.infer<typeof CustomWorkflowDetailsSchema>;
export type ValidatedCreateWorkflowPayload = z.infer<typeof CreateWorkflowPayloadSchema>;
export type ValidatedCreateWorkflowStepPayload = z.infer<typeof CreateWorkflowStepPayloadSchema>;
export type ValidatedCustomWorkflowQuery = z.infer<typeof CustomWorkflowQuerySchema>;
export type ValidatedCustomWorkflowStepQuery = z.infer<typeof CustomWorkflowStepQuerySchema>;

// Form schema for custom workflow
export const customWorkflowFormSchema = (initialData: CustomWorkflowDetails | null) => {
    return z.object({
...
export const customWorkflowStepFormSchema = z.object({
    ...
    defaultInstructionText: z.string().optional(),
    defaultDueDateOffsetDays: z.number().min(0, "عدد الأيام يجب أن يكون أكبر من أو يساوي 0").default(0),
});

// Define the form values types
export type CustomWorkflowFormValues = z.infer<ReturnType<typeof customWorkflowFormSchema>>;
export type CustomWorkflowStepFormValues = z.infer<typeof customWorkflowStepFormSchema>;

// Utility function to format workflow data for API calls
export const formatWorkflowPayload = (formData: CustomWorkflowFormValues, id?: string) => {
    const payload = {
        ...formData,
    };
...
export const formatWorkflowStepPayload = (formData: CustomWorkflowStepFormValues, workflowId: string, id?: string) => {
    const payload = {
        ...formData,
        workflowId,
    };
...
export const getCorrespondenceTypeDisplay = (type: CorrespondenceTypeEnum): string => {
    const typeNames = {
