...
import {
  createLeaveWorkflowStepSchema,
  type CreateLeaveWorkflowStepFormValues
} from '../utils/leave-workflow-step';
...
import { leaveWorkflowStepService } from '../api/leave-workflow-step.service';
...
interface LeaveWorkflowStepFormDialogProps {
  leaveRequestId: string;
  trigger?: React.ReactNode;
  defaultValues?: Partial<CreateLeaveWorkflowStepFormValues>;
}

export function LeaveWorkflowStepFormDialog({
  leaveRequestId,
  trigger,
  defaultValues
}: LeaveWorkflowStepFormDialogProps) {
  ...
  const form = useForm<CreateLeaveWorkflowStepFormValues>({
    resolver: zodResolver(createLeaveWorkflowStepSchema),
    defaultValues: {
  ...
  function onFormSubmit(data: CreateLeaveWorkflowStepFormValues) {
    setLoading(true);
    ...
    authApiCall(() => leaveWorkflowStepService.createLeaveWorkflowStep(payload))
      .then((result) => {
        if (result?.data) {
          toast.success('تم إنشاء إجراء تحويل بنجاح');
          setOpen(false);
          form.reset({
            leaveRequestId: leaveRequestId,
            actionType: ActionTypeEnum.RequestApproval,
          ...
          toast.error('حدث خطأ أثناء إنشاء إجراء تحويل');
        }
      })
      .catch(() => {
        toast.error('حدث خطأ أثناء إنشاء إجراء تحويل');
      ...
      <DialogTrigger asChild>
        {trigger || (
          <Button variant='outline' className='w-full'>
            ...
            إنشاء إجراء تحويل جديد
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className='max-h-[90vh] w-[600px] overflow-y-auto'>
        <DialogHeader>
          <DialogTitle>إنشاء إجراء تحويل جديد</DialogTitle>
          <DialogDescription>
            املأ النموذج أدناه لإنشاء إجراء تحويل جديد
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          ...
          >
            ...
            <div className='space-y-4 rounded-lg border p-4'>
              <h3 className='text-lg font-medium'>إجراء تحويل</h3>
