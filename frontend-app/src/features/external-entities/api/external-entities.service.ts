...
export const externalEntitiesService = {
    async getExternalEntities(query: IExternalEntityQuery) {
        try {
            const response = await axiosInstance.get(`${baseUrl}/ExternalEntity/GetExternalEntityList`, { params: query });
    ...
    async getExternalEntityById(id: string) {
        ...
        try {
            const response = await axiosInstance.get(`${baseUrl}/ExternalEntity/GetExternalEntityById/${id}`);
    ...
    async createExternalEntity(entity: IExternalEntityPayload) {
        try {
            const response = await axiosClient.post(`${baseUrl}/ExternalEntity/CreateExternalEntity`, entity);
            ...
            return {
                succeeded: false,
                data: false,
                message: error?.response?.data?.message || 'Failed to create external entity',
    ...
    async updateExternalEntity(entity: IExternalEntityPayload) {
        ...
        try {
            const response = await axiosClient.put(`${baseUrl}/ExternalEntity/UpdateExternalEntity`, entity);
            return response.data as IResponse<boolean>;
        } catch (error: any) {
            ...
            return {
                succeeded: false,
                data: false,
                message: error?.response?.data?.message || 'Failed to update external entity',
