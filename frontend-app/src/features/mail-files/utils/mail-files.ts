...
export const correspondenceTypeLabels: Record<
  CorrespondenceType,
...
export const formatMailFilePayload = (
  formData: MailFileFormValues,
  id?: string
) => {
  const payload = {
    ...formData
...
export const getCorrespondenceTypeText = (type: number): string => {
  return (
    correspondenceTypeLabels[type as CorrespondenceType]?.label || 'Unknown'
  );
};
