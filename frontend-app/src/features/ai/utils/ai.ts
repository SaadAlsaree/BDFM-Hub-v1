 ...
 */
export async function genAIResponse(
  options: AIResponseOptions
): Promise<Response> {
  try {
    // Extract the latest user message for the query
    const userMessages = options.data.messages.filter(
      (msg) => msg.role === 'user'
    );
    ...
    if (!ragResponse) {
      ...
      console.warn(
        'AI Service: No response from RAG service, returning mock response'
      );
 ...
 */
export async function genAIResponseStream(
  options: AIResponseOptions
): Promise<Response> {
