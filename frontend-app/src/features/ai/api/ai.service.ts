...
export const aiService = {
  ...
  ): Promise<IResponse<RAGResponse> | null> {
    try {
      ...
      if (response.status >= 400) {
        console.error(
          'AI Service: RAG query failed with status:',
          response.status,
          response.statusText
        );
      ...
      if (error instanceof Error && error.message?.includes('timeout')) {
        console.error(
          'AI Service: Request timeout - AI processing is taking longer than expected'
        );
      }
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        console.error(
          'AI Service: Network error - API endpoint may not be available'
        );
      }
  ...
  async queryRAGStream(request: SearchRequest): Promise<ReadableStream | null> {
    try {
      ...
      if (!response.ok) {
        console.error(
          'AI Service: RAG stream query failed with status:',
          response.status,
          response.statusText
        );
        return null;
      ...
      console.error(
        'AI Service: Error querying RAG streaming endpoint:',
        error
      );
