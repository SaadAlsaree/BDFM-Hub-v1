...
interface LeaveRequestRejectDialogProps {
  leaveRequest: LeaveRequest;
}

export default function LeaveRequestRejectDialog({
  leaveRequest
}: LeaveRequestRejectDialogProps) {
  ...
  const onSubmit = async (data: RejectLeaveRequestFormValues) => {
    try {
      setLoading(true);

      const response = await authApiCall(() =>
        leaveRequestService.rejectLeaveRequest(data)
      );
      ...
      if (response?.succeeded) {
        ...
        toast.error('لم يتم رفض طلب الإجازة!');
      }
    } catch (error) {
      console.error(error);
      toast.error('حدث خطأ!');
