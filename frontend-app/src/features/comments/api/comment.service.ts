...
import { ICommentList, ICommentDetails, ICommentCreate, ICommentUpdate, ICommentDelete } from '@/features/comments/types/comments';
...
export const commentService = {
    ...
    async getComments(correspondenceId: string) {
        try {
            const response = await axiosClient.get(`${baseUrl}/Comments/GetCommentsListByCorrespondenceId/${correspondenceId}/correspondence?includeReplies=true`);
    ...
    async createComment(comment: ICommentCreate) {
        try {
            const response = await axiosClient.post(`${baseUrl}/Comments/CreateComment`, comment);
            if (response.status >= 400) {
    ...
    async updateComment(comment: ICommentUpdate) {
        try {
            const response = await axiosClient.put(`${baseUrl}/Comments/UpdateComment`, comment);
            if (response.status >= 400) {
    ...
    async deleteComment(comment: ICommentDelete) {
        try {
            const response = await axiosClient.delete(`${baseUrl}/Comments/DeleteComment`, { data: comment });
            if (response.status >= 400) {
