...
export function StatisticsOverview({ filters }: StatisticsOverviewProps) {
  const [corrStats, setCorrStats] = useState<CorrespondenceStatistics | null>(
    null
  );
  const [workflowStats, setWorkflowStats] = useState<WorkflowStatistics | null>(
    null
  );
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 gap-6 lg:grid-cols-2'>
        ...
        <Card className='p-6'>
          <h3 className='mb-4 text-lg font-semibold'>توزيع الكتب</h3>
          <div className='space-y-4'>
            {/* By Type */}
            <div>
              <p className='text-muted-foreground mb-2 text-sm'>حسب النوع</p>
            ...
            {/* By Priority */}
            <div>
              <p className='text-muted-foreground mb-2 text-sm'>حسب الأولوية</p>
              <div className='space-y-2'>
                {Object.entries(corrStats.byPriority).map(
                  ([priority, count]) => (
                    <div
                      key={priority}
        ...
        {/* Workflow Breakdown */}
        <Card className='p-6'>
          <h3 className='mb-4 text-lg font-semibold'>توزيع خطوات العمل</h3>
          <div className='space-y-4'>
            {/* By Status */}
            <div>
              <p className='text-muted-foreground mb-2 text-sm'>حسب الحالة</p>
              <div className='space-y-2'>
                {Object.entries(workflowStats.byStatus).map(
                  ([status, count]) => (
                    <div
                      key={status}
                      className='flex items-center justify-between'
                ...
                {Object.entries(workflowStats.byAction)
                  ...
                  .map(([action, count]) => (
                    <div
                      ...
                      className='flex items-center justify-between'
                    >
                      <span className='truncate text-sm'>{action}</span>
                      <Badge variant='secondary'>{count}</Badge>
                    </div>
                  ))}
              </div>
