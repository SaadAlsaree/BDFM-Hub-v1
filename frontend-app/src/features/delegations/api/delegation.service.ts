...
export const delegationService = {
    async getDelegationList(query: Record<string, any>) {
        try {
            const response = await axiosInstance.get(`${baseUrl}/Delegation/GetDelegationList`, { params: query });
    ...
    async getDelegationDetail(id: string) {
        try {
            const response = await axiosInstance.get(`${baseUrl}/Delegation/GetDelegationById/${id}`);
            if (response.status >= 400) {
    ...
    async createDelegation(delegation: CreateDelegationPayload) {
        try {
            const response = await axiosClient.post(`${baseUrl}/Delegation/CreateDelegation`, delegation);
            if (response.status >= 400) {
    ...
    async updateDelegation(delegation: CreateDelegationPayload) {
        try {
            const response = await axiosClient.put(`${baseUrl}/Delegation/UpdateDelegation`, delegation);
            if (response.status >= 400) {
    ...
    async updateDelegationStatus(id: string, status: number) {
        try {
            const response = await axiosClient.patch(`${baseUrl}/Delegation/UpdateDelegationStatus`, { id, status });
            if (response.status >= 400) {
    ...
    async deleteDelegation(id: string) {
        try {
            const response = await axiosClient.delete(`${baseUrl}/Delegation/DeleteDelegation/${id}`);
            if (response.status >= 400) {
