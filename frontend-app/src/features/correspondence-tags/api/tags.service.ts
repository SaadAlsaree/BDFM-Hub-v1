...
///BDFM/v1/api/Tag/CreateTag
 async createTag(payload: CorrespondenceTag) {
    try {
        const response = await axiosClient.post(`${baseUrl}/Tag/CreateTag`, payload);
 ...
 async updateTag(payload: CorrespondenceTag) {
    try {
        const response = await axiosClient.put(`${baseUrl}/Tag/UpdateTag`, payload);
        if (response.status >= 400) {
 ...
 async softDeleteTag(id: string) {
    try {
        const response = await axiosClient.delete(`${baseUrl}/Tag/SoftDeleteTag/SoftDelete/${id}`);
        if (response.status >= 400) {
 ...
 async getTagList(query: Record<string, any>) {
    try {
        const response = await axiosClient.get(`${baseUrl}/Tag/GetTagList`, { params: query });
        if (response.status >= 400) {
 ...
 async getCorrespondencesWithTags(query: Record<string, any>) {
    try {
        const response = await axiosInstance.get(`${baseUrl}/Tag/GetCorrespondencesWithTags`, { params: query });
        if (response.status >= 400) {
            console.error('Error getting correspondences with tags:', response.statusText);
            return null;
        }
