...
export function useTodoManagement({ workflowStepId, initialTodos = [] }: UseTodoManagementProps) {
    ...
    // Add new todo
    const addTodo = useCallback(async (todoData: Omit<WorkflowStepTodoPayload, 'id'>) => {
        try {
            setLoading(true);
            setError(null);

            const response = await authApiCall(() =>
                correspondenceService.createWorkflowStepTodo(todoData as WorkflowStepTodoPayload)
            );
    ...
    const updateTodo = useCallback(async (todoId: string, updates: Partial<WorkflowStepTodoPayload>) => {
        try {
            setLoading(true);
            setError(null);
    ...
    const stats = {
        ...
        completionRate: todos.length > 0 ? (todos.filter(t => t.isCompleted).length / todos.length) * 100 : 0
    };
