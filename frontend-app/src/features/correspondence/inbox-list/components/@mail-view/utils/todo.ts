...
export const todoFormSchema = (initialData: WorkflowStepTodoPayload | null) =>
    z.object({
        title: z.string().min(1, 'العنوان مطلوب').max(200, 'العنوان يجب أن يكون أقل من 200 حرف'),
...
export const convertPayloadToFormValues = (todo: WorkflowStepTodoPayload): TodoFormValues => {
    return {
        title: todo.title || '',
