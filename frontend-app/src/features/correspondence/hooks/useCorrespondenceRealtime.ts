...
export function useCorrespondenceRealtime() {
  ...
  const { onNotificationEvent, offNotificationEvent, isConnected } =
    useSignalR();
  ...
  const handleCorrespondenceAssignedToModule = useCallback(
    (notification: {
      ...
      queryClient.setQueryData(
        ['correspondence', notification.correspondenceId],
        (oldData: any) => {
          if (oldData) {
            return {
              ...oldData,
  ...
  const handleCorrespondenceStatusChangedWithNotification = useCallback(
    (notification: {
      correspondenceId: string;
      newStatus: string;
      message: string;
  ...
  const handleWorkflowStepCreated = useCallback(
    (notification: {
      ...
      queryClient.invalidateQueries({
        queryKey: ['workflow-steps', notification.correspondenceId]
      });
      queryClient.invalidateQueries({
        queryKey: ['correspondence', notification.correspondenceId]
      });
    },
    [queryClient]
  );
