...
export class CorrespondenceSaver {
  ...
  async save(data: CorrespondenceSaveData): Promise<SaveResult> {
    try {
      ...
      const correspondence = await this.saveCorrespondence(
      ...
      if (data.linkingData?.linkedCorrespondenceId) {
        const linkSuccess = await this.linkCorrespondence(
          correspondence,
          data.linkingData
        );
        if (!linkSuccess) {
      ...
      if (data.attachmentsData?.length) {
        const attachmentSuccess = await this.uploadAttachments(
          correspondence,
      ...
      if (errors.length === 0) {
        toast.success('تم تسجيل المراسلة بنجاح!');
      } else {
        toast.success('تم تسجيل المراسلة بنجاح مع بعض التحذيرات');
      }
      ...
      console.error('Correspondence save error:', error);
  ...
  private async saveCorrespondence(
    data: ExternalCorrespondencePayload
  ): Promise<string | null> {
    try {
      const response = await this.authApiCall(() =>
        correspondenceService.createRegisterIncomingExternalMail(data)
      );
  ...
  private async uploadAttachments(
    ...
    try {
      ...
      console.log(
        `Uploaded ${successfulUploads} out of ${attachments.length} attachments`
      );
