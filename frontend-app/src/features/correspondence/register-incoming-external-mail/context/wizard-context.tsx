...
export function WizardProvider({
  ...
  const [formData, setFormData] = useState<Partial<WizardFormData>>(() => {
    if (initialData) {
      return {
        externalReferenceNumber: initialData.externalReferenceNumber || '',
        externalReferenceDate: initialData.externalReferenceDate || '',
        originatingExternalEntityId:
          initialData.originatingExternalEntityId || '',
        originatingSubEntities: initialData.originatingSubEntities || [],
  ...
  const previousStep = () => {
    if (currentStep > WizardStep.EXTERNAL_INFO) {
      setCurrentStep(currentStep - 1);
    }
  };

  const isFirstStep = () => currentStep === WizardStep.EXTERNAL_INFO;
  const isLastStep = () => currentStep === WizardStep.ATTACHMENTS;

  const resetWizard = () => {
    setCurrentStep(WizardStep.EXTERNAL_INFO);
