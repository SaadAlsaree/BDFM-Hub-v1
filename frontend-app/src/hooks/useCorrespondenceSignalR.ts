...
export function useCorrespondenceSignalR(
  config: CorrespondenceSignalRConfig = {}
) {
  ...
  const invalidateCorrespondenceQueries = useCallback(
  ...
  const createDefaultHandler = useCallback(
    (eventName: string) => {
      return (data: any) => {
          ...
          switch (eventName) {
            ...
            case 'CorrespondenceStatusChanged':
              if (configRef.current.enableToastNotifications) {
                ...
                toast.info(
                  `تغيرت حالة المراسلة من ${oldStatus} إلى ${newStatus}`
                );
              }
              invalidateCorrespondenceQueries(data.correspondenceId);
              break;
            ...
            case 'CorrespondenceAssignedToModule':
              if (configRef.current.enableToastNotifications) {
                const message =
                  data.message || `تم تعيين مراسلة جديدة: ${data.moduleName}`;
            ...
            case 'CorrespondenceStatusChangedWithNotification':
              if (configRef.current.enableToastNotifications) {
                const message =
                  data.message || `تحديث الحالة: ${data.newStatus}`;
                toast.info(message);
              }
              invalidateCorrespondenceQueries(data.correspondenceId);
              break;
  ...
  const createCombinedHandler = useCallback(
    (eventName: keyof SignalREvents, userHandler?: (data: any) => void) => {
  ...
  useEffect(() => {
    ...
    eventsToListen.forEach((eventName) => {
      try {
        const userHandler = config[eventName] as
          | ((data: any) => void)
          | undefined;
