...
import { Notification, NotificationQuery, NotificationType } from '@/types/notifications';
...
export function useNotifications(query: NotificationQuery = {}, options: { skipGlobalListeners?: boolean } = {}) {
    ...
    const {
        ...
        queryFn: async () => {
            return await authApiCall(async () =>
                await notificationsApi.getUserNotificationsClient(query)
            );
        },
    ...
    const markAsReadMutation = useMutation({
        ...
        onSuccess: (result, id) => {
            if (result?.succeeded) {
                ...
                queryClient.setQueryData(unreadCountKey, (prev: number) => Math.max(0, (prev || 0) - 1));
        ...
        try {
            ...
            queryClient.setQueryData(queryKey, (oldData: any) => {
                ...
                return {
                    ...
                    data: {
                        ...oldData.data,
                        items: [newNotification, ...oldData.data.items].slice(0, query.pageSize || 50), // Limit cache size
    ...
    }, [queryClient, queryKey, isNotificationTypeEnabled, preferences, playNotificationSound, showBrowserNotification, debouncedInvalidateNotifications, query.pageSize]);
    ...
    useEffect(() => {
        ...
        if (options.skipGlobalListeners) {
            console.log('🔔 [useNotifications] Skipping event listeners - using global listeners');
            return;
        }
        ...
        const correspondenceAssignedHandler = (data: any) => {
            console.log('🔔 [useNotifications] Received CorrespondenceAssignedToModule event:', JSON.stringify(data, null, 2));
        ...
        const statusChangedHandler = (data: any) => {
            console.log('🔔 [useNotifications] Received CorrespondenceStatusChangedWithNotification event:', JSON.stringify(data, null, 2));
            handleNewNotification({
        ...
        const workflowStepHandler = (data: any) => {
            console.log('🔔 [useNotifications] Received WorkflowStepCreated event:', JSON.stringify(data, null, 2));
            handleNewNotification({
        ...
        const workflowAssignedHandler = (data: any) => {
            console.log('🔔 [useNotifications] Received WorkflowStepAssigned event:', JSON.stringify(data, null, 2));
            handleNewNotification({
    ...
    }, [onNotificationEvent, offNotificationEvent, handleNewNotification, isSignalRAvailable, options.skipGlobalListeners]);
