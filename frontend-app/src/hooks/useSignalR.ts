...
export function useSignalR() {
    ...
    if (session?.error === 'RefreshAccessTokenError' || session?.error) {
      ...
      if (signalRRef.current) {
        ...
        if (connectionStateHandlerRef.current) {
          signalRRef.current.offConnectionStateChange(
  ...
  const offNotificationEvent = useCallback(
    <K extends keyof SignalREvents>(
      eventName: K,
      callback: SignalREvents[K]
    ) => {
  ...
  const getConnectionDiagnostics = useCallback(() => {
    ...
    const signalRDiagnostics = signalRRef.current
      ? signalRRef.current.getConnectionDiagnostics()
      : {
          reconnectAttempts: 0,
