...
{
    public class GetUserIsStarredHandler : GetAllWithCountHandler<Correspondence, StarredItemVm, GetUserIsStarredQuery>, IRequestHandler<GetUserIsStarredQuery, Response<PagedResult<StarredItemVm>>>
    {

        ICurrentUserService _currentUserService;
        public GetUserIsStarredHandler(IBaseRepository<Correspondence> repository, ICurrentUserService currentUserService) : base(repository)
        {
            _currentUserService = currentUserService;
        }

        public override Expression<Func<Correspondence, StarredItemVm>> Selector => x => new StarredItemVm
        {
        ...
        public async Task<Response<PagedResult<StarredItemVm>>> Handle(GetUserIsStarredQuery request, CancellationToken cancellationToken)
        {
            ...
            if (!result.Any())
                return ErrorsMessage.NotFoundData.ToErrorMessage<PagedResult<StarredItemVm>>(null!);

            // Set StatusName for each item (if applicable)
            result.ToList().ForEach(x =>
            {
                // Note: StarredItemVm doesn't have Status property, so this might not be needed
            ...
            var pagedResult = new PagedResult<StarredItemVm>
            {
                Items = result,
                TotalCount = count
            };
