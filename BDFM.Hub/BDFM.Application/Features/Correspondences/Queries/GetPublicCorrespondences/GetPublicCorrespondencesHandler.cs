...
{
    internal class GetPublicCorrespondencesHandler : GetAllWithCountHandler<Domain.Entities.Core.Correspondence, GetPublicCorrespondencesVm, GetPublicCorrespondencesQuery>, IRequestHandler<GetPublicCorrespondencesQuery, Response<PagedResult<GetPublicCorrespondencesVm>>>
    {
        ...
        public GetPublicCorrespondencesHandler(IBaseRepository<Correspondence> repository, ICurrentUserService currentUserService, IPermissionValidationService permissionValidationService, IBaseRepository<User> userRepository) : base(repository)
        {
        ...
        public override Expression<Func<Correspondence, GetPublicCorrespondencesVm>> Selector => x => new GetPublicCorrespondencesVm
        {
        ...
        public async Task<Response<PagedResult<GetPublicCorrespondencesVm>>> Handle(GetPublicCorrespondencesQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                query = query.Where(c => c.CreateAt >= request.CreatedDate.Value);
            }

            if (request.MailDate.HasValue)
            {
                query = query.Where(c => c.MailDate <= request.MailDate.Value);
            }

            // Filter for public correspondences only and apply user access control
            query = query.Where(c =>
                c.CorrespondenceType == CorrespondenceTypeEnum.Public &&
                (
